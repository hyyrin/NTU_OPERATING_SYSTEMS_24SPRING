...
bmap(struct inode *ip, uint bn)
{
  ...
  bn -= NDIRECT;
  //singely-indirect blocks
  if(bn < 6 * NINDIRECT){
    // Load indirect block, allocating if necessary.
      uint index = bn / NINDIRECT;
      uint offset = bn % NINDIRECT;
      if ((addr = ip->addrs[NDIRECT + index]) == 0) ip->addrs[NDIRECT + index] = addr = balloc(ip->dev);
    ...
    if (bn < NDOUINDIRECT){
        ...
        uint *bpa = (uint *)bpt->data;
        // two layers
        uint layer_first = bn / NINDIRECT;
        uint layer_second = bn % NINDIRECT;
...
itrunc(struct inode *ip)
{
  ...
  for(i = 0; i < NDIRECT; i++){
    ...
    for (int k = NDIRECT; k < NDIRECT + 6; k++){
        if(ip->addrs[k]){
            bp = bread(ip->dev, ip->addrs[k]);
            a = (uint*)bp->data;
    ...
    if (ip->addrs[NDIRECT + 6]){
        bpt = bread(ip->dev, ip->addrs[NDIRECT + 6]);
        b = (uint *)bpt->data;
